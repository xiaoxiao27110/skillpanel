...
- **Multi-session build** → `/to-spec` to turn the thread into a spec, then `/to-tickets` to split it into tracer-bullet tickets, each declaring its **blocking edges**. Then hand the ticket directory to `/implement` in this session — it orchestrates one sub-agent per ticket, verifying and committing each, while the user watches a single conversation.
